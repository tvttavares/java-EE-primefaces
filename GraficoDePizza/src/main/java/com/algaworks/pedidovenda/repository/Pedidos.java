...
public class Pedidos implements Serializable {
	...
	public Map<String, BigDecimal> valorUsuario() {
		...
		criteria.setProjection(Projections.projectionList().add(Projections.groupProperty("vendedor").as("usuario"))
				.add(Projections.sum("valorTotal").as("valor")));

		List<UsuarioValor> valores = criteria.setResultTransformer(Transformers.aliasToBean(UsuarioValor.class)).list();

		for (UsuarioValor usuarioValor : valores) {
			resultado.put(usuarioValor.getUsuario().getNome(), usuarioValor.getValor());
		}
